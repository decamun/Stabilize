...
void calibrateGyro(void)
{
  ...
  //Serial.println("Gyro Calibration Factor: " + String(gyro_calibration_factor));
}
...
void motSet() {
  ...
  mot_client2.AngleKp.Set(com2,AngleKp);
  mot_client1.AngleKd.Set(com1,AngleKd);
  mot_client2.AngleKd.Set(com2,AngleKd);
}
...
// Puts an absolute angle request message in the outbound com queue
  mot_client1.ObsAbsoluteAngle.Get(com1);
  mot_client2.ObsAbsoluteAngle.Get(com2);
  ...
  // Grab outbound messages in the com queue, store into buffer
  // If it transferred something to communication_buffer...
  if(com2.GetTxBytes(communication_buffer_2,communication_length_2))
  {
    // Use Arduino serial hardware to send messages
    Serial4.write(communication_buffer_2,communication_length_2);
    //Serial.println("Grabbed outbound messages from motor2");
  }
  ...
  while(com1.PeekPacket(&rx_data,&rx_length))
  {
    ...
    // Reads however many bytes are currently available
  communication_length_2 = Serial4.readBytes(communication_buffer_2, Serial4.available());
  // Puts the recently read bytes into com's receive queue
  com2.SetRxBytes(communication_buffer_2,communication_length_2);

  rx_length = 0;

  // while we have message packets to parse
  while(com2.PeekPacket(&rx_data,&rx_length))
  {
    // Remember time of received packet
    communication_time_last = millis();
    
    // Share that packet with all client objects
    mot_client2.ReadMsg(com2,rx_data,rx_length);

    // Once we're done with the message packet, drop it
    com2.DropPacket();
  }

  mot_client1.CmdAngle.Set(com1,roll_angle_rad);
  mot_client2.CmdAngle.Set(com2,pitch_angle_rad);
